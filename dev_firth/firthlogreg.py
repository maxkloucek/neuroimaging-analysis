...
class FirthLogisticRegression(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        # all of this stuff doesnt happen
        # let's just keep it but move it to the bottom...?
        # no comment out, make it as simple as possible!
        # if not self.skip_ci:
        #     if not self.wald:
...
def _firth_newton_raphson(X, y, max_iter, max_stepsize, max_halfstep, tol, mask=None):
    ...
    for iter in range(1, max_iter + 1):
        ...
        fisher_info_mtx = XW.T @ XW # looks good besides this fisher info! X not XW?
        hat = _hat_diag(XW)
        ...
        steps = 0  # this bit gets skipped when loglikie = inf! i.e. overflow error!
        # print(iter, loglike, loglike_new)
        # print(step_size[0:3])
        # comparing with infinites is a bit of a mess...
        # print(loglike < loglike_new)
        # print(iter, loglike, loglike_new, loglike < loglike_new)
        # print(coef_new[0:3])
        # anyway, I don't fully understand this code so it would be a little rouge
        # to use it blidnly.... if I get a postdoc then this would be good foundation!
        while loglike < loglike_new:
            # print('hi', steps, loglike, loglike_new)
            step_size *= 0.5
            # print(step_size[0:3])
            coef_new = coef + step_size
            preds_new = expit(X @ coef_new)
            loglike_new = _loglikelihood(X, y, preds_new)
            # print(steps, loglike_new)
            steps += 1
        ...
        # print('')
        # it doesn't!
        # there's no reason for the last loglike_new to be -inf still...
        # so hard to figure out someone elsese code..
        # print(iter, loglike_new) # I don't understand why its' always -inf..
        # anyway, it converges and that's all I care about!
        # home time!
        # print(np.linalg.norm(coef_new - coef) < tol, np.linalg.norm(coef_new - coef))
...
# alternative to below, need to get super sure on this!
# print(np.linalg.det(np.matmul(X.T, X)), X.shape)
# fisher_info_mtx = XW.T @ X
# W = np.sqrt(preds * (1 - preds))
# W = np.diag(W)
# XTW = np.matmul(X.T, W)
# FI = np.matmul(XTW, X)
# print(FI.shape)
# print(np.all(FI == fisher_info_mtx))
# print(FI[0:5, 0:5], fisher_info_mtx[0:5, 0:5])
...
def _loglikelihood(X, y, preds):
    ...
    # ok so thes are the same! I can try to numba matplutiply maybe?
    # somethings done a bit wrong!
    # FI = np.matmul(XTW, X)
    # print(FI.shape)
    fisher_info_mtx = XW.T @ XW
    # det = np.linalg.det(fisher_info_mtx)
    # penalty = 0.5 * np.log(det)
    sign, logdet = np.linalg.slogdet(fisher_info_mtx)
    penalty = sign * (0.5 * logdet) # my version using the log for overflow
    # can already check this on N=200!
    # print(penalty == penalty_me) # wook ok sick!!
...
def _get_XW(X, preds, mask=None):
    ...
    # W = np.sqrt(preds * (1 - preds))
    # W = np.diag(W)
    # XW = W.T @ X # this is same as np.matmul! -> no numab suport for np.matmul

    return XW

# this is the big time consuming function!
# @njit
def _hat_diag(XW):
...
# only used in _profile_likelihood_ci
